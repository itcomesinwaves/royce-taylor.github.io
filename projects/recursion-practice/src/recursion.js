...
var range = function(x, y, rangeNums=[]) {
  // base
 if (x - y === -1 || x === y || x - y === 1) {
  return rangeNums;
...
// powerOfTwo(16); // true
// powerOfTwo(10); // false
//do n = n/2 iteratively. In any iteration, if n%2 becomes non-zero and n is not 1 
...
if (string[0].toUpperCase() === string[string.length-1].toUpperCase() && string.length > 1) {
  // recursion    
...
/*
multiply(2, 4) 2 * 4 is the same as 
2 + 2 + 2 + 2//
multiple(-2, 4) -2 *4 is the same as
-2 + -2 + -2 + -2
...
var rMap = function(array, callback, outArr=[]) {
// base
if (array.length === 0) {
  return outArr;
}
...
//recursion
outArr.push(callback(array[0])); 
return rMap(array.slice(1), callback, outArr);
...
var nthFibo = function(n, fib=[0, 1]) {
  //base
  if (n === 0) {// checking for the number 0 
    return fib[fib.length-2];
  } else if (n === 1) {// checking for the number 1
    return fib[fib.length-1];
  } else if (n < 0) {
    return null; 
  }
  //recursion
  fib.push(fib[fib.length - 1] + fib[fib.length - 2]); // always pushing a new n
...
let capitalized = splitted[0].toUpperCase();    
...
// base
// if (list.length === 0) {
//   return array; /// alternative way to break out of the iteration, prevents infinite loop if an empty list is passed in
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4] [-2, -7]
...
return numToText(str.substring(1), newStr);
};
